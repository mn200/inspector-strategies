...
int HPC_sparsemv_CSR(const int nrow, const int* nnzInRow, const int* cols, 
                    const double *vals, 
		    const double * const x, double * const y)
{
  // rowStart will complete CSR notation
  int * rowStart = (int *)malloc(sizeof(int)*(nrow+1));
  rowStart[0] = 0;
...
int HPC_sparsemv( HPC_Sparse_Matrix *A, 
		 const double * const x, double * const y)
{
  /* // for checking vs original
  double * myy = (double *) malloc (sizeof(double)*A->local_nrow);

  HPC_sparsemv_CSR(A->local_nrow, A->nnz_in_row, A->list_of_inds,
                   A->list_of_vals, x, myy);
  */
...
#pragma omp parallel for
  ...
  /*
  cout << "~~~~~~~~~~~~ Checking spmv differences ~~~~~~~~~~~~~~\n";
  for (int i=0; i< nrow; i++)
    {
      if (abs(y[i] - myy[i]) > 0.000001) {
        cout << "DIFF:  " << abs(y[i] - myy[i]) << endl;
      }
    }
