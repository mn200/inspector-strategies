...
open listTheory
open pairTheory
open actionTheory dagTheory
open cardinalTheory

(* nag = "Nested Action Graph" *)
val _ = new_theory "ndag";
...
val FOLDR_add_action_SURJ = store_thm(
  ...
  ``SURJ (FOLDR dagAdd ε) UNIV UNIV``,
  simp[SURJ_DEF] >> ho_match_mp_tac dag_ind >> conj_tac
  >- (qexists_tac `[]` >> simp[]) >>
  simp[PULL_EXISTS] >> map_every qx_gen_tac [`l`, `a`] >>
  qexists_tac `a::l` >> simp[]);

val INFINITE_actions = store_thm(
  "INFINITE_actions[simp]",
  ``INFINITE univ(:(α,β)node)``,
...
val nodes_cardEQ_CROSS = store_thm(
  "nodes_cardEQ_CROSS",
  ``univ(:(α,β)node) ≈ univ(:β list) × univ(:α)``,
  `univ(:(α,β) node) ≈ univ(:β list) × univ(:β list) × univ(:α) ∧
   univ(:β list) × univ(:β list) × univ(:α) ≈ univ(:β list) × univ(:α)`
    suffices_by metis_tac[cardeq_TRANS] >>
  conj_tac
  >- (simp[BIJ_DEF, cardeq_def] >>
      qexists_tac `λa. ((a.reads, a.writes), a.data)` >>
      simp[INJ_DEF, SURJ_DEF] >> conj_tac >- simp[action_component_equality, oneTheory.one] >>
      simp[FORALL_PROD] >> qx_genl_tac [`rds`, `wts`, `data`] >>
      qexists_tac `<| reads := rds; writes := wts; data := data; ident := ()|>` >>
      simp[]) >>
  `univ(:β list) × univ(:β list) ≈ univ(:β list)`
    suffices_by metis_tac[CARDEQ_CROSS,cardeq_REFL] >>
  simp[SET_SQUARED_CARDEQ_SET]);

val tau = ``:(β list # δ) inf``
...
val univblist = prove(
  ...
  qexists_tac `λbl : β list. INR (bl, ARB)` >>
  simp[]);
...
val tau_dag = ``:(^tau, β) dag``
val aqtau_dag = ty_antiq tau_dag

val aglt = MATCH_MP cardeqLleq (CONJ (ONCE_REWRITE_RULE [cardeq_SYM] eqn1) c12)

val sumlt1 = prove(
  ``UNIV : (^aqtau_dag + δ) set <<= {T;F} CROSS UNIV : ^aqtau set``,
  simp[cardleq_def, INJ_DEF] >>
  strip_assume_tac (SIMP_RULE (srw_ss()) [INJ_IFF, cardleq_def] aglt) >>
  qexists_tac `λs. case s of INL t => (T,f t) | INR d => (F, INR(ARB,d))` >>
...
val lemma2 = prove(
   ``UNIV : ^aqtau set <<= UNIV : (^aqtau_dag + δ) set``,
  simp[cardleq_def, INJ_DEF] >>
  qexists_tac
    `λd. INL (<| writes := []; reads := []; data := d; ident := ARB |> <+ ε)` >>
  simp[])
...
val (cwf_rules, cwf_ind, cwf_cases) = Hol_reln`
  (∀d. cwf (cmk (INR d))) ∧
  (∀g. (∀d. d ∈ IMAGE adata (nodeset g) ⇒ cwf d) ⇒ cwf (cmk (INL g)))
`;
...
val cwfdmapR_total = store_thm(
  ...
  fs[GSYM RIGHT_EXISTS_IMP_THM, SKOLEM_THM, PULL_EXISTS] >>
  qexists_tac `gf g (dagmap f g)` >>
  match_mp_tac (CONJUNCT2 dR_rules) >> simp[] >> rpt strip_tac >>
  pop_assum mp_tac >> qid_spec_tac `g` >>
  ho_match_mp_tac dag_ind >> simp[dagREL_rules]);

val cmk_11 = prove(
  ``(cmk x = cmk y) ⇔ x = y``,
  metis_tac[mkdest_def]);

val dagREL_unique = store_thm(
  "dagREL_unique",
  ``∀d1 d2. dagREL (λa b. R a b ∧ ∀b'. R a b' ⇒ b' = b) d1 d2 ⇒
            ∀d2'. dagREL R d1 d2' ⇒ (d2' = d2)``,
  Induct_on `dagREL` >> conj_tac >- simp[Once dagREL_cases] >>
  qx_genl_tac [`d1`, `d2`, `a1`, `a2`] >> strip_tac >>
  dsimp[dagREL_elim_add] >> rpt strip_tac >>
  simp[action_component_equality, oneTheory.one]);
...
val cwfdmapR_unique = store_thm(
  ...
  Induct_on `dagREL` >> conj_tac
  >- dsimp[Once cwfdmapR_cases, cmk_11] >>
  qx_genl_tac [`g`, `rg`, `a1`, `a2`] >> strip_tac >>
  simp[Once cwfdmapR_cases, cmk_11, PULL_EXISTS, dagREL_elim_add] >>
  qx_genl_tac [`b`, `d0`] >> strip_tac >>
  `b = a2` by simp[action_component_equality, oneTheory.one] >>
  simp[] >>
  `d0 = rg` suffices_by simp[] >>
  metis_tac[dagREL_unique]);
...
val cwfdmapf_INL = prove(
  ``cwf (cmk (INL g)) ⇒
    cwfdmapf df dg (cmk (INL g)) = dg g (dagmap (cwfdmapf df dg) g)``,
  ...
  Q.UNDISCH_THEN `cwf (cmk (INL g))` mp_tac >> pop_assum mp_tac >>
  map_every qid_spec_tac [`rg`, `g`] >> Induct_on `dagREL` >>
  simp[] >> rw[] >> pop_assum mp_tac >> simp[Once cwf_cases] >>
  dsimp[cmk_11] >> pop_assum mp_tac >>
  simp[Once cwf_cases, SimpL ``$==>``] >> dsimp[cmk_11] >> rpt strip_tac >>
  fs[] >> simp[polydata_upd_def, action_component_equality, oneTheory.one] >>
  simp[cwfdmapf_def] >> SELECT_ELIM_TAC >>
  metis_tac[cwfdmapR_total, cwfdmapR_unique]);

val nnode as {term_ABS_t,term_REP_t,...} =
    newtypeTools.rich_new_type ("nnode", cwf_nonempty)

val nnodeREC_def = Define`
  nnodeREC df dg n = cwfdmapf df (\g. dg (dagmap ^term_ABS_t g)) (^term_REP_t n)
`;
...
val DG_def = Define`
  DG (g : ((α,β)nnode, α) dag) =
   ^term_ABS_t (cmk (INL (dagmap ^term_REP_t g)))
`;

val IN_dagmap = store_thm(
  "IN_dagmap",
  ``∀d. x ∈ dagmap f d ⇔ ∃y. y ∈ d ∧ x = polydata_upd f y``,
  ho_match_mp_tac dag_ind >> dsimp[]);

val nnodeREC_DG = store_thm(
  "nnodeREC_DG[simp]",
  ``nnodeREC df dg (DG g) = dg g (dagmap (nnodeREC df dg) g)``,
  simp[nnodeREC_def, DG_def] >>
  `cwf (cmk (INL (dagmap nnode_REP g)))`
    by (match_mp_tac (CONJUNCT2 cwf_rules) >> simp[PULL_EXISTS, IN_dagmap] >>
        simp[polydata_upd_def, #termP_term_REP nnode]) >>
  simp[#repabs_pseudo_id nnode] >>
  simp[cwfdmapf_INL, cmk_11, dagmap_dagmap_o] >>
  asm_simp_tac (srw_ss() ++ ETA_ss)
    [GSYM nnodeREC_def, combinTheory.o_DEF, #absrep_id nnode]);

val _ = type_abbrev("ndag0", ``:(('rws, 'd)nnode, 'rws) dag``)

val nnodeSize_def = Define `
  nnodeSize = nnodeREC (K 1) (λg r. 1 + dagSIGMA I r)
`;

val nnodeSize_thm = store_thm(
  "nnodeSize_thm[simp]",
  ``nnodeSize (DN d) = 1 ∧
    nnodeSize (DG g) = 1 + dagSIGMA I (dagmap nnodeSize g)``,
  simp[nnodeSize_def, nnodeREC_DN, nnodeREC_DG]);

val nagSize_def = Define`
  nagSize (g:(α,β)ndag0) = dagSIGMA nnodeSize g
`;
...
val nnode_ind = store_thm(
  "nnode_ind",
  ``∀P. (∀d. P (DN d)) ∧
        (∀g. (∀d. d ∈ IMAGE adata (nodeset g) ⇒ P d) ⇒
             P (DG g)) ⇒
        ∀n. P n``,
  simp[DN_def, DG_def, PULL_EXISTS, forall_nnode] >>
  ntac 2 strip_tac >> Induct_on `cwf` >>
  simp[PULL_EXISTS] >> qx_gen_tac `g` >>
  first_x_assum (qspec_then `dagmap nnode_ABS g` mp_tac) >>
  simp[IN_dagmap, PULL_EXISTS, polydata_upd_def]  >>
  rpt strip_tac >>
  `dagmap (nnode_REP o nnode_ABS) g = dagmap I g`
    by (match_mp_tac dagmap_CONG >>
