...
open pred_setTheory forLoopTheory monadicPrimitivesTheory
open lcsymtacs
open monadsyntax

val CONJ_ss = boolSimps.CONJ_ss

val _ = new_theory "simpleMLoop";
...
val vsub_simple_update_out_range_mFOR = store_thm(
  "vsub_simple_update_out_range_mFOR",
  ``∀i A. hi ≤ vsz A ∧ (i < lo ∨ hi ≤ i) ⇒ 
          mFOR (lo,hi) (λi a. update a i (g i a)) A '' i = A ' i``,
  rpt strip_tac >> DEEP_INTRO_TAC mFOR_RULE >>
  qexists_tac `λj a. vsz A = vsz a ∧ a ' i = A ' i` >>
  asm_simp_tac (srw_ss() ++ ARITH_ss ++ CONJ_ss) 
    [update_assert, vsub_assert, listTheory.EL_LUPDATE]);
...
val vsub_simple_update_in_range_FOR = store_thm(
  ...
  ``∀i A. lo ≤ i ∧ i < hi ∧ hi ≤ vsz A ⇒
          mFOR (lo,hi) (λj a. update a j (g j)) A '' i = SOME (g i)``,
  rpt strip_tac >> DEEP_INTRO_TAC mFOR_RULE >>
  qexists_tac `λj a. vsz a = vsz A ∧ ∀k. lo ≤ k ∧ k < j ⇒ a ' k = SOME (g k)` >>
  asm_simp_tac (srw_ss() ++ ARITH_ss)[] >>
  map_every qx_gen_tac [`j`, `a`] >> 
  asm_simp_tac (srw_ss() ++ ARITH_ss ++ CONJ_ss) 
    [update_assert, vsub_assert, listTheory.EL_LUPDATE] >>
  ntac 2 (rw[]));
...
val PERMS_suffice0 = prove(
  ``BIJ Δ (count N) (count N) ∧
    (final = mFOR (0, N) (\i a. update a i (g i)) A1) ⇒
    ∀m n A2.
      (vsz A1 = vsz A2) ∧ N ≤ vsz A1 ∧
      (m = N - n) ∧ n ≤ N ∧
      (∀i. A2 ' i = if i ∈ IMAGE Δ (count n) then final '' i
                    else A1 ' i)
     ⇒
      (mFOR (n, N) (\i a. update a (Δ i) (g (Δ i))) A2 = final)``,
  strip_tac >> Induct >| [
